...
class MotionCueView @JvmOverloads constructor(
    ...
    enum class ColorMode { PURE_DARK, AUTO_TONE, INVERT }
    ...
    private var colorMode: ColorMode = ColorMode.AUTO_TONE
    ...
    private var dotSampleColor = IntArray(0)
    private var dotSampleReady = BooleanArray(0)
    private var dotColorTarget = IntArray(0)
    private var dotColorCurrent = IntArray(0)
    ...
    fun setAutoTone(enabled: Boolean) {
        colorMode = if (enabled) ColorMode.AUTO_TONE else ColorMode.PURE_DARK
    }
    ...
    private fun rebuildDots(w: Int, h: Int) {
        ...
        dotSampleColor = IntArray(dots.size) { Color.BLACK }
        dotSampleReady = BooleanArray(dots.size) { false }
        dotColorTarget = IntArray(dots.size) { Color.BLACK }
        dotColorCurrent = IntArray(dots.size) { Color.BLACK }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        runCatching {
            ...
            for ((i, dot) in dots.withIndex()) {
                ...
                val sampleColor = sampleColorIfNeeded(i, dot.edge, cx, cy, radius, widthF, heightF, nowNs)
                when (colorMode) {
                    ColorMode.AUTO_TONE -> {
                        updateToneFromSample(i, sampleColor, nowMs)
                        ...
                        dotColorTarget[i] = Color.rgb(shade, shade, shade)
                    }
                    ColorMode.PURE_DARK -> {
                        dotToneTarget[i] = 0
                        dotToneCurrent[i] += (dotToneTarget[i].toFloat() - dotToneCurrent[i]) * toneLerp
                        dotColorTarget[i] = Color.BLACK
                    }
                    ColorMode.INVERT -> {
                        // Invert mode: use per-dot sampled background color and render inverse color.
                        // If sampling is unavailable, gracefully fall back to auto tone logic.
                        if (sampleColor != null) {
                            dotColorTarget[i] = invertRgb(sampleColor)
                            val luma = bt601Luma(sampleColor) / 255f
                            lastDebug = lastDebug.copy(quickLuminance = luma)
                        } else {
                            updateToneFromSample(i, null, nowMs)
                            dotToneCurrent[i] += (dotToneTarget[i].toFloat() - dotToneCurrent[i]) * toneLerp
                            val tone = dotToneCurrent[i].coerceIn(0f, 1f)
                            val shade = if (tone >= 0.5f) 255 else 0
                            dotColorTarget[i] = Color.rgb(shade, shade, shade)
                ...
                dotColorCurrent[i] = lerpColor(dotColorCurrent[i], dotColorTarget[i], toneLerp)
                ...
                fillPaint.color = dotColorCurrent[i]
                fillPaint.alpha = alpha
                canvas.drawCircle(cx, cy, max(1f, radius), fillPaint)

                if (i == 0) {
                    val l = dotLumaSmooth[i] / 255f
                    val rendered = dotColorCurrent[i]
                    val contrastBlack = DotColorSystem.contrastRatio(Color.BLACK, rendered)
                    val contrastWhite = DotColorSystem.contrastRatio(Color.WHITE, rendered)
                    val contrast = max(contrastBlack, contrastWhite)
                    lastDebug = ColorRuntimeDebug(
                        quickLuminance = l,
                        chosenStyle = when (colorMode) {
                            ColorMode.PURE_DARK -> "pure-dark"
                            ColorMode.AUTO_TONE -> if (Color.red(rendered) >= 128) "light-dot" else "dark-dot"
                            ColorMode.INVERT -> "invert-dot"
                        },
    ...
    private fun sampleColorIfNeeded(
        index: Int,
        edge: Edge,
        x: Float,
        y: Float,
        ...
        if (nowNs - dotLastSampleNs[index] >= sampleIntervalNs) {
            ...
            val sampled = sampleAroundDot(edge, x, y, radius, screenW, screenH)
            if (sampled != null) {
                dotSampleColor[index] = sampled
                dotSampleReady[index] = true
            }
        }
        return if (dotSampleReady[index]) dotSampleColor[index] else null
    }

    private fun updateToneFromSample(index: Int, sampleColor: Int?, nowMs: Long) {
        val color = sampleColor ?: run {
            dotToneTarget[index] = 0
            return
        }
        val luma = bt601Luma(color)
    ...
    private fun invertRgb(color: Int): Int {
